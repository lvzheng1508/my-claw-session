...
这不是谁更聪明,而是两种工程路线的自然延伸:Cursor 倾向于用基础设施降低对模型单次处理超大代码库的依赖;Claude Code 则倾向于直接暴露原始文件,再用压缩工程保证模型能持续看到它该看到的东西。
...
更接近这一取向的实践包括 Cursor 的部分方案、OpenClaw 的部分抽象设计,以及一些企业级应用实践。
...
前文已经建立了一个分析框架:上下文腐烂、记忆断裂、自我评估偏差——这是 Harness Engineering 的"三个关键问题"。本质上,各家面对的是同一组问题,只是在用不同的方式回答。没有哪个框架"发明"了全新的问题,也没有哪个框架"解决"了所有问题。理解了这一点,新项目层出不穷这件事就不再令人困惑——它们只是同一个问题空间中的不同解法。任何一个 Harness Agent,无论它叫什么名字、用什么架构,都可以用这套分析框架进行评估。
...
### 8.3 Harness 对业务的启示:明确问题,对症下药

本文讨论的所有 Harness Engineering 实践——上下文压缩、记忆管理、生成-评估分离——核心问题只有一个:**如何让一个大语言模型在复杂、长时间的任务中可靠地运行。** 这是一个工程问题。

但实际业务中的痛点,往往有不同的来源。用文中 Harness 的"三个问题"做类比,业务场景中常见的瓶颈可以归纳为另一个"三角":

- **数据质量**——喂给模型的数据一团糟,模型再强也输出不了好结果。这是数据治理的范畴
- **业务流程**——智能客服的转接规则、知识库边界、回答不了时该怎么升级处理,这些都没有定义清楚。这是业务设计的范畴
- **评估标准**——不知道"好"的标准是什么,就无法判断模型输出是否达标。这是产品定义的范畴

这两个"三角"对应的是不同层面的问题。Harness 的价值在于,它提供了一个明确的诊断框架——当你能清楚地说出瓶颈在"上下文腐烂"还是"数据质量",就已经迈出了最重要的一步。
