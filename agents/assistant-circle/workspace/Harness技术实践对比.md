...
**Harness Engineering 不是流程驱动,而是推理驱动。**

整个系统的驱动力是模型的推理能力--每一次 tool call、每一个决策、每一步任务推进,都是模型在推理之后做出的选择。Harness Engineering 不指挥模型做什么,它做的是确保 Agent Loop 不偏离目标:
...
理解这一点很重要。它意味着 Harness Engineering 的设计原则不是"用工程替代模型",而是"用工程释放模型"--让模型的推理能力在复杂、长时间的任务中得到最大程度的发挥。
...
### 补充说明:Harness Engineering ≠ Workflow Orchestrator
...
### 3.2 Self-Summarization:训练级别的压缩实践
...
Cursor 的 Self-Summarization 直接从模型训练层面解决了 Context Rot 问题。与 Claude Code 的 prompt-level 多层压缩不同,Cursor 选择在训练时就把"如何有效压缩"教给模型。

两种路线代表了 Harness Engineering 中"压缩"问题的两种解法:

| 维度 | Claude Code(prompt-level) | Cursor(training-level) |
|------|---------------------------|------------------------|
| **实现方式** | 四层压缩系统(AutoCompact + MicroCompact + SessionMemory + Compaction) | 训练时将摘要链纳入奖励信号 |
| **优势** | 不依赖训练基础设施,可快速迭代 | token 效率更高,可处理超长轨迹 |
| **劣势** | 效率天花板受限于 prompt 设计 | 需要训练资源和 RL 基础设施 |
| **适用场景** | 通用场景,快速落地 | 大规模、长周期任务 |
...
这意味着 Codex 的 Harness 不需要在客户端自行实现压缩和状态管理,而是将这些能力下沉到 API 服务端,由 Responses API 的基础设施直接提供。
...
interface ContextEngine {
  ...
  maintain?(params: { sessionId, sessionFile, runtimeContext }): Promise<MaintenanceResult>;
  ...
  prepareSubagentSpawn?(params: { parentSessionKey, childSessionKey, ttlMs }): Promise<SubagentSpawnPreparation>;
...
如果说前三张表对比的是"怎么解决问题",这张表关注的是另一个维度:**各家工程系统的复杂度分布**。
...
向量索引方案也存在时效性问题:代码频繁变动时,索引可能与实际文件不一致。Claude Code 直接读原始文件,不存在这个问题--代码改了,下次读就是新的。大规模并行任务且代码库相对稳定时,索引更高效;个人开发且代码在密集修改中时,直接读文件更可靠。

| 维度 | Claude Code | Cursor | OpenClaw |
|------|-------------|--------|----------|
| **整体架构复杂度** | 相对精简:主要本地运行,单进程 | 更庞大:云端 VM、向量索引、训练基础设施、大规模并发调度 | 轻量:本地/自托管,插件化 |
| **单点工程极限** | 更极致:四层压缩系统、Bash 分段权限检查、Agent Memory 三级存储,将 prompt-level 工程做到这个范式的天花板 | 相对克制:问题更多拆给基础设施解决(索引、训练级优化、VM 隔离) | 实用:可插拔接口,够用即可 |
| **设计目标** | 个人开发者的最佳 AI 协作工具 | 自动化工程规模(数千 Agent 并行一周) | 通用 AI 助理网关 |
| **代码理解方式** | 直接读原始文件(无预索引) | 云端向量索引 + 语义搜索 | 依赖子代理的文件读取能力 |
| **压缩压力** | 极高(所有内容走 context window) | 较低(索引预过滤减少进入 context 的内容) | 中等(单会话规模有限) |
| **代码时效性** | 实时(直接读文件,无缓存问题) | 存在索引过时风险(频繁变动时索引与实际文件可能不一致) | 实时(直接读文件) |

**观察**:Claude Code 和 Cursor 代表了两种截然不同的工程美学--前者在单点上把工程做到极致,后者用基础设施的复杂度换取规模能力。OpenClaw 则是另一个路线:不追求极致,追求通用和灵活。
